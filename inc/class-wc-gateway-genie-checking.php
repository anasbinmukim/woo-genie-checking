   ...
   */
     ...
     */
		public function process_genie_checking_order(){
			...
			// Get Order ID
			$order_id = 0;
			if(isset($callback_value[0])){
				$order           	= explode('=', $callback_value[0]);
				$order_id    = ( $order[1] ) ? $order[1] : 0;
			}

			// Get Order Status
			$status = '';
			if(isset($callback_value[1])){
				$status_detail   	= explode('=', $callback_value[1]);
				$status    = ( $status_detail[1] ) ? $status_detail[1] : '';
			}

			// Request No
			$requestno = '';
			if(isset($callback_value[3])){
				$requestno_detail   = explode('=', $callback_value[3]);
				$requestno    = ( $requestno_detail[1] ) ? $requestno_detail[1] : 0;
			}

			// Transaction Result 0 for success
			$result = '';
			if(isset($callback_value[4])){
				$result_detail   	= explode('=', $callback_value[4]);
				$result    = ( $result_detail[1] ) ? $result_detail[1] : '';
			}

			// Retured Message
			$msg = '';
			if(isset($callback_value[5])){
				$msg_detail   		= explode('=', $callback_value[5]);
				$msg    = ( $msg_detail[1] ) ? $msg_detail[1] : '';
			}

			$this->update_genie_order($order_id, $result, $requestno, $msg, $status);
